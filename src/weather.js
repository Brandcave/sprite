...
export class Weather {
  constructor({ scene, renderer, sun, sky, fill, windUniforms = [] }) {
    ...
    // Grit on the wind. Pale rather than sand-coloured — over the beach, actual
    // sand colour is invisible, and this has to read against both.
    this.dustField = new ParticleField(scene, {
      count: 500, color: 0xf2e8d2, fall: 0.5, len: 0.16, width: 0.055,
      radius: 16, height: 6,
    });
  ...
  update(dt, dayT, focus) {
    ...
    this.dustField.update(this.clock, focus, {
      amount: this.wind * 0.9,
      opacity: 0.5 * this.wind,
      drift: this.drift.clone().multiplyScalar(1.5),
    });
  }
  ...
  updateWind(dt) {
    ...
    for (const u of this.windUniforms) {
      ...
      u.uWindScale.value = 1 + 1.8 * this.wind;
      u.uWindBend.value = this.wind;
      u.uWindDir.value.copy(this.dir);
    }
  }
